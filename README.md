<h1 align="center">Clément ROUVROY</h1>
<div align="center">

I'm studying <em>Computer Sciences</em> at <a href="https://www.ens.psl.eu/">ENS-PSL</a>.<br/>

## Projects on this GitHub

### Nanyang Technological University, Singapore

From my internship at NTU, supervised by Prof. CONG Gao and PhD. SHI Jiachen.
<ul>
  <li>First version of a <a href="https://github.com/CRouvroy/QHICS">What-If</a> for Column-oriented database.</li>
</ul>

### ENS Paris, PSL University, France
(projects are made with Nathan Boyer and Grégoire Le Corre, some only with Grégoire, some only with Nathan, some with both)
<ul>
<li>A <a href="https://github.com/CRouvroy/AnaNAS">DeepQ-RL model applied to a car trying to learn to drive</a>.</li>
<li>A <a href="https://github.com/CRouvroy/DiffusionModel">Diffusion Model</a> presentation and application. The slides go up to ControlNet (in a nutshell), the code only implements the first paper by Ho et al.</li>
<li>A <a href="https://github.com/orgs/Git-Ane/repositories">GIT developed in C++</a>, it supports server, branches, merges, etc.</li>
<li>A <a href="https://github.com/greglcr/GeometricDL">Graph network applied to detect fake news</a>.</li>
</ul>

</div>
